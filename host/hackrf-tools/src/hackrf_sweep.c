...
#define TRIGGERING_TIMES (50)						 // 3600
...
char longitude_code[1]; // Longitude code where the antenna is located { E | W }
float latitude = 0; // Latitude code where the antenna is located
char latitude_code[1]; // Latitude code where the antenna is located { N | S }
float altitude = 0; // Altitude in meters
char obj[50]; // Object observed
char content[100]; // Content
...
FILE *hackrfLogsFile = NULL;
char hackrfLogsPath[90];
...
sighandler(int signum)
{
	...
	{
		fprintf(hackrfLogsFile, , "Caught signal %d\n", signum);
		do_exit = true;
		return TRUE;
	}
	return FALSE;
...
{
	fprintf(hackrfLogsFile, "hackrf_sweep | sigint_callback_handler |Caught signal %d\n", signum);
	do_exit = true;
}
#endif
...
int hackRFTrigger()
{
	fprintf(hackrfLogsFile, "hackrf_sweep | hackRFTrigger() | Triggering Start\n");
	if (signal(SIGALRM, timerHandler) == SIG_ERR)
	{
		fprintf(hackrfLogsFile, "timer | hackRFTrigger | Unable to catch alarm signal");
		return EXIT_FAILURE;
	}

	if (setitimer(ITIMER_REAL, &timer, 0) == -1)
	{
		fprintf(hackrfLogsFile, "timer | hackRFTrigger | Error calling timer");
		return EXIT_FAILURE;
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | hackRFTrigger() | Execution Success\n");
	return EXIT_SUCCESS;
}
...
//			fprintf(hackrfLogsFile, "\n"); // TODO: Delete
			if (counterSucess == 0)
			{
				...
				if (checkOrderFreqs(nRanges, ((double)(frequency) / FREQ_ONE_MHZ)) == EXIT_FAILURE)
				{
					return EXIT_FAILURE;
				}
			...
			if ((uint64_t)(frequency + ((step_value * sampleRate * 3) / 4)) >= freq_max * FREQ_ONE_MHZ) // Where the sweep finished
			{
				...
				counterSucess++;
				//fprintf(hackrfLogsFile, "hackrf_sweep | rx_callback() | Data Caught. Iteration %d finished\n", counterSucess);
...
{
	fprintf(hackrfLogsFile, "hackrf_sweep | checkParams() | Checking if parameters values are correct\n");

	if (lna_gain % 8)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | checkParams() | warning: lna_gain (-l) must be a multiple of 8\n");
	}

	if (vga_gain % 2)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | checkParams() | warning: vga_gain (-g) must be a multiple of 2\n");
	}
	...
	if (amp)
	{
		if (amp_enable > 1)
		{
			fprintf(hackrfLogsFile, "hackrf_sweep | checkParams() | argument error: amp_enable shall be 0 or 1.\n");
			usage();
			return EXIT_FAILURE;
		}
	}
	...
	if (antenna)
	{
		if (antenna_enable > 1)
		{
			fprintf(hackrfLogsFile, "hackrf_sweep | checkParams() | argument error: antenna_enable shall be 0 or 1.\n");
			usage();
			return EXIT_FAILURE;
		}
	}
	...
	if (fftSize < FFTMIN)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | checkParams() | argument error: FFT bin width (-w) must be no more than 5000000\n");
		return EXIT_FAILURE;
	}

	if (fftSize > FFTMAX)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | checkParams() | argument error: FFT bin width (-w) must be no less than 2445\n");
		return EXIT_FAILURE;
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | checkParams() | Execution Success\n");
	return EXIT_SUCCESS;
}
...
{
	fprintf(hackrfLogsFile, "hackrf_sweep | initConfigHackRF() | Initialize configuration of HackRF One\n");
	result = hackrf_init();
	if (result != HACKRF_SUCCESS)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | initConfigureHackRF() | hackrf_init() failed: %s (%d)\n", hackrf_error_name(result), result);
		usage();
		return EXIT_FAILURE;
	}
	...
	if (result != HACKRF_SUCCESS)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | initConfigureHackRF() | hackrf_open() failed: %s (%d)\n", hackrf_error_name(result), result);
		usage();
		return EXIT_FAILURE;
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | initConfigHackRF() | Execution Success\n");
	return result;
}
...
{
	fprintf(hackrfLogsFile, "hackrf_sweep | setHackRFParams() | call hackrf_sample_rate_set(%.03f MHz)\n",
			((float)sampleRate / (float)FREQ_ONE_MHZ));
	result = hackrf_set_sample_rate_manual(device, sampleRate, 1);
	if (result != HACKRF_SUCCESS)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | setHackRFParams() | hackrf_sample_rate_set() failed: %s (%d)\n",
				hackrf_error_name(result), result);
		usage();
		return EXIT_FAILURE;
	}

	fprintf(hackrfLogsFile, "setHackRFParams | call hackrf_baseband_filter_bandwidth_set(%.03f MHz)\n",
	...
	if (result != HACKRF_SUCCESS)
	{
		fprintf(hackrfLogsFile, "setHackRFParams | hackrf_baseband_filter_bandwidth_set() failed: %s (%d)\n",
				hackrf_error_name(result), result);
		usage();
		return EXIT_FAILURE;
	}
	...
	fprintf(hackrfLogsFile, "hackrf_sweep | setHackRFParams() | Execution Success\n");
	return result;
}
...
static int setSweeping()
{
	...
	if (result != HACKRF_SUCCESS)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | setSweeping() | hackrf_init_sweep() failed: %s (%d)\n",
				hackrf_error_name(result), result);
		return EXIT_FAILURE;
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | setSweeping() | Execution Sucess\n");
	return EXIT_SUCCESS;
}
...
static int sweeping()
{
	if (hackRFTrigger() == EXIT_FAILURE)
	{
		return EXIT_FAILURE;
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | sweeping() | ===SWEEPING STARTED===\n");
	fprintf(hackrfLogsFile, "hackrf_sweep | Start triggering %d times\n", TRIGGERING_TIMES);

	result |= hackrf_start_rx_sweep(device, rx_callback, NULL); // rx callback write are the values to save in the img
	if (result != HACKRF_SUCCESS)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | sweeping() | hackrf_start_rx_sweep() failed: %s (%d)\n", hackrf_error_name(result), result);
		return EXIT_FAILURE;
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | sweeping() | ===SWEEPING DONE===\n");
	return EXIT_SUCCESS;
}
...
static float sweepDuration()
{
	...
	fprintf(hackrfLogsFile, "hackrf_sweep | sweepDuration()\n");
	...
	while ((hackrf_is_streaming(device) == HACKRF_TRUE) && (do_exit == false))
	{
		...
		if (durationSweep >= 1.0f)
		{
			...
			/*fprintf(hackrfLogsFile, "%" PRIu64 "hackrf_sweep | sweepDuration() | total sweeps completed, %.2f sweeps/second\n",
...
{
	fprintf(hackrfLogsFile, "hackrf_sweep | checkAvailabilityAmpOption() | Checking availabilty amp option\n");

	if (amp)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | checkAvailabilityAmpOption() | call hackrf_set_amp_enable(%u)\n", amp_enable);
		result = hackrf_set_amp_enable(device, (uint8_t)amp_enable);
		if (result != HACKRF_SUCCESS)
		{
			fprintf(hackrfLogsFile, "hackrf_sweep | checkAvailabilityAmpOption() | hackrf_set_amp_enable() failed: %s (%d)\n",
					hackrf_error_name(result), result);
			usage();
			return EXIT_FAILURE;
		}
	...
	fprintf(hackrfLogsFile, "hackrf_sweep | checkAvailabilityAmpOption() | Execution Success\n");
	return EXIT_SUCCESS;
}
...
{
	fprintf(hackrfLogsFile, "hackrf_sweep | checkAvailabilityAntennaOption() | Checking availability of antenna option\n");
	if (antenna)
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | checkAvailabilityAntennaOption() | call hackrf_set_antenna_enable(%u)\n", antenna_enable);
		result = hackrf_set_antenna_enable(device, (uint8_t)antenna_enable);
		if (result != HACKRF_SUCCESS)
		{
			fprintf(hackrfLogsFile, "hackrf_sweep | checkAvailabilityAntennaOption() | hackrf_set_antenna_enable() failed: %s (%d)\n",
					hackrf_error_name(result), result);
			usage();
			return EXIT_FAILURE;
		}
	...
	fprintf(hackrfLogsFile, "hackrf_sweep | checkAvailabilityAntennaOption() | Execution Success\n");
...
{
	fprintf(hackrfLogsFile, "hackrf_sweep() | checkStreaming() | Checking streaming status\n");
	...
	fprintf(hackrfLogsFile, "hackrf_sweep() | checkStreaming() | Execution Success\n");
}
...
{
	fprintf(hackrfLogsFile, "hackrf_sweep | endConnection() | Ending connection with HackRF One\n");
	if (device != NULL)
	{
		result = hackrf_close(device);
		if (result != HACKRF_SUCCESS)
		{
			fprintf(hackrfLogsFile, "hackrf_sweep | endConnection() | hackrf_close() failed: %s (%d)\n",
					hackrf_error_name(result), result);
			return EXIT_FAILURE;
		}
		...
		{
			fprintf(hackrfLogsFile, "hackrf_sweep | endConnection() | hackrf_close() done\n");
		}

		hackrf_exit();
		fprintf(hackrfLogsFile, "hackrf_sweep | endConnection() | hackrf_exit() done\n");
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | endConnection() | Execution Success\n");
	return EXIT_SUCCESS;
}
...
static int runConfiguration()
{
	...
	if (initConfigureHackRF() == EXIT_FAILURE)
	{
		return EXIT_FAILURE;
	}
	...
	fprintf(hackrfLogsFile, "hackrf_sweep | runConfiguration() | HackRF One configuration DONE.\n");
	...
	// save frequency data
	if (saveFrequencies(freq_min, freq_max, nRanges, step_range) == EXIT_FAILURE)
	{
		return EXIT_FAILURE;
	}
	if (saveTimeSteps() == EXIT_FAILURE)
	{
		return EXIT_FAILURE;
	}
	...
	{
		fprintf(hackrfLogsFile, "hackrf_sweep | runConfiguration() | Was not possible to allocate memory for power samples\n");
		return EXIT_FAILURE;
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | runConfiguration() | Memory allocated for power samples\n");
	fprintf(hackrfLogsFile, "hackrf_sweep | runConfiguration() | Execution Success\n");
	fprintf(hackrfLogsFile, "=============================================================\n\n");
	return EXIT_SUCCESS;
}
...
{
	fprintf(hackrfLogsFile, "=============================================================\n");
	fprintf(hackrfLogsFile, "hackrf_sweep | runExecution() | Execution starts\n");
	...
	checkStreaming();
	fprintf(hackrfLogsFile, "hackrf_sweep| Total sweep completed successfully: %d out of %d\n", counterSucess, TRIGGERING_TIMES);

	fprintf(hackrfLogsFile, "hackrf_sweep | All triggering actions(%d) where completed\n", TRIGGERING_TIMES);
	fprintf(hackrfLogsFile, "hackrf_sweep | Total duration of sweeping: %.2f s ||", totalDuration);
	...
	fprintf(hackrfLogsFile, "=============================================================\n");
	fprintf(hackrfLogsFile, "hackrf_sweep | runExecution() | Execution Success\n");
...
static int runGeneration(struct tm localTimeFirst, struct tm localTimeLast)
{
	...
	fprintf(hackrfLogsFile, "=============================================================\n");
	fprintf(hackrfLogsFile, "hackrf_sweep | runGeneration() | Execution starts\n");
	...
	fprintf(hackrfLogsFile, "hackrf_sweep | runGeneration() | reorganizeSamples() | Reorganizing Samples Started\n");
	for (i = 0; i < nRanges; i++)
	{
		if ((i + 1) != real_order_frequencies[i] && flagsOrder[i] == 0) // i+1 is the position that should have
		{
			if (reorganizeSamples(i + 1, real_order_frequencies[i], nRanges,
								  ...
								  nElements, fftSize / 4, naxes[1]) == EXIT_FAILURE)
			{
				return EXIT_FAILURE;
			}
		}
	...
	for (i = 0; i < nRanges; i++)
	{
		if (flagsOrder[i] == 0)
		{
			fprintf(hackrfLogsFile, "hackrf_sweep | runGeneration() | reorganizeSamples() | Flag[%d] is down. Reorganization of samples failed\n", i);
			return EXIT_FAILURE;
		}
	}

	fprintf(hackrfLogsFile, "hackrf_sweep | runGeneration() | reorganizeSamples() | Execution Success;\n");
	...
	{
		if (writeHackrfDataIntoTxtFiles(localTimeFirst, localTimeLast, samplesOrdered, frequencyDatas, timeSteps) == EXIT_FAILURE)
		{
			return EXIT_FAILURE;
		}
	}
	...
	fprintf(hackrfLogsFile, "=============================================================\n");
	fprintf(hackrfLogsFile, "hackrf_sweep | runGeneration() | Execution Success\n");
	return EXIT_SUCCESS;
}
...
int main(int argc, char **argv)
{
	...
	if (setvbuf(hackrfLogsFile, NULL, _IOFBF, FD_BUFFER_SIZE) != EXIT_SUCCESS)
	{
		return EXIT_FAILURE;
	}
	...
	if (runGeneration(tm_timeStartSweeping, tm_timeEndSweeping) == EXIT_FAILURE)
	{
		return EXIT_FAILURE;
	}
	...
	freeFFTMemory();
	freeFitsMemory();
	fprintf(hackrfLogsFile, "hackrf_sweep | The dynamic memory used was successfully released.\nEND.\n");
	...
	if (generationMode == 0)
	{
		strcat(extraInfo, "Python Generation");
	}
	else
	{
