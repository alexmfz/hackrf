...
/*LIBRERIAS API hackRF*/
#include "/home/manolo/hackrf/host/libhackrf/src/hackrf.h" //Librería hackrf (API)
/**********/
...
/*LIBRERIA UNIX*/
#include <getopt.h> //Librería UNIX que coge los valores de la linea de comandos 
/**********/
...
#include <windows.h> //Windows API & macros
#ifdef _MSC_VER
...
/*Funcion que te calcula la hora del dia*/
...
/*******CONSTANTES DE HACKRF ONE API*******/

#define FD_BUFFER_SIZE (8*1024) //Tamaño buffer file descriptor

/*Pasos entre fmax y fmin*/
#define FREQ_ONE_MHZ (1000000ull) // 1 MHz

/*Frecuencias minima y máxima*/
#define FREQ_MIN_MHZ (0)    /*    0 MHz */ //MIN de la API
#define FREQ_MAX_MHZ (7250) /* 7250 MHz */ //MAX de la API 
...
#define TUNE_STEP (DEFAULT_SAMPLE_RATE_HZ / FREQ_ONE_MHZ) //20MHz/1MHz = 20 pasos
#define OFFSET 7500000 //offset de 7.5 MHz
...
/*Se encarga de calcular una diferencia temporal*/
static float TimevalDiff(const struct timeval *a, const struct timeval *b) {
   return (a->tv_sec - b->tv_sec) + 1e-6f * (a->tv_usec - b->tv_usec);
}

/*Convierte una cadena en un numero hexadecimal o binario*/
int parse_u32(char* s, uint32_t* const value) {
	...
	s_end = s;
	ulong_value = strtoul(s, &s_end, base); //convierte la cadena s a un unsigned long de base 2 o 16
	if( (s != s_end) && (*s_end == 0) ) {
...
int parse_u32_range(char* s, uint32_t* const value_min, uint32_t* const value_max) {
	...
	char *sep = strchr(s, ':'); //Busca la primera concurrencia de un caracter en una cadena
	if (!sep) //sino hay da error
		return HACKRF_ERROR_INVALID_PARAM; 
...
/*Te pasa la potencia en dbs*/
float logPower(fftwf_complex in, float scale) //==> In = Real + Imaginario
{   
	float re = in[0] * scale;
	float im = in[1] * scale;
	float magsq = re * re + im * im; //no es el modulo. Faltaria la raiz
	return (float) (log2(magsq) * 10.0f / log2(10.0f)); // ==> Equivale a 10*logbase10(maqsq)
}

/*Funcion callback de recepción de datos*/
int rx_callback(hackrf_transfer* transfer) {
	...
	gettimeofday(&usb_transfer_time, NULL); //calcula el momento del dia
	byte_count += transfer->valid_length; //por cada recepción aumenta el contador de bytes
	buf = (int8_t*) transfer->buffer; //Se asigna el buffer de la estructura transfer
	ifft_bins = fftSize * step_count; //fftSize = 20, step_count = rango de frecuencias/Tam. paso

	//La información total se transmite en bloques de tamaño transfer->buffer
	for(j=0; j<BLOCKS_PER_TRANSFER; j++) { 
		ubuf = (uint8_t*) buf;
		if(ubuf[0] == 0x7F && ubuf[1] == 0x7F) { //Porque esa condicion? NO ENTIENDO ESA PARTE
			frequency = ((uint64_t)(ubuf[9]) << 56) | ((uint64_t)(ubuf[8]) << 48) | ((uint64_t)(ubuf[7]) << 40)
					...
					| ((uint64_t)(ubuf[3]) << 8) | ubuf[2]; //en ese caso la frecuencia se ve desplazada (n-2)*8. Por ejemplo (9-2)*8 = 56
		} else {
			buf += BYTES_PER_BLOCK;
			continue;
		}
		if (frequency == (uint64_t)(FREQ_ONE_MHZ*frequencies[0])) {
			if(sweep_started) {
				if(ifft_output) {
					fftwf_execute(ifftwPlan); //ejecuta fft
					//genera muestras reales e imagarias y las guarda
					for(i=0; i < ifft_bins; i++) {
						ifftwOut[i][0] *= 1.0f / ifft_bins; //real 
						ifftwOut[i][1] *= 1.0f / ifft_bins; //imaginario
		...
		if((FREQ_MAX_MHZ * FREQ_ONE_MHZ) < frequency) { //7250 MHz < frequency
			buf += BYTES_PER_BLOCK;
			continue;
		}
		...
		if(binary_output) {
			...
			fprintf(outfile, "%s.%06ld, %" PRIu64 ", %" PRIu64 ", %.2f, %u",
					...
					(uint64_t)(frequency+DEFAULT_SAMPLE_RATE_HZ/4), //45MHz + 5MHz
					fft_bin_width,
					fftSize);
			for(i = 0; (fftSize / 4) > i; i++) {
			...
			fprintf(outfile, "%s.%06ld, %" PRIu64 ", %" PRIu64 ", %.2f, %u",
					time_str,
					(long int)usb_transfer_time.tv_usec,
					(uint64_t)(frequency+(DEFAULT_SAMPLE_RATE_HZ/2)), //55MHz
					(uint64_t)(frequency+((DEFAULT_SAMPLE_RATE_HZ*3)/4)), //45 + 15 
					fft_bin_width,
					fftSize);
			for(i = 0; (fftSize / 4) > i; i++) {
...
static hackrf_device* device = NULL; //dispositivo hackrf one
...
int main(int argc, char** argv) {
	...
	while( (opt = getopt(argc, argv, "a:f:p:l:g:d:n:N:w:1BIr:h?")) != EOF ) {
		...
		case 'f': //seleccionar frecuencias
			...
			if(MAX_SWEEP_RANGES <= num_ranges) { //rangos de frecuencia. Por ejemplo 240:260 es 1 rango
				fprintf(stderr,
	...
	for (i = 0; i < fftSize; i++) {
		window[i] = (float) (0.5f * (1.0f - cos(2 * M_PI * i / (fftSize - 1)))); //ventanado
	}
...
/**Una vez se deja preparado el entorno se pone en ejecución**/
    

	//Se inicializa entorno
	result = hackrf_init();
	if( result != HACKRF_SUCCESS ) {
	...
	//Tras inicializarlo se abre para usarse por el puerto serie
	...
	//Se abre el archivo donde realizar la escritura
...
//Handler de señales
	...
	//Se establece un ancho de banda como filtro banda base
	...
	//Se establece una ganancia en VGA y LNA
	...
	//Se inicia el sweep del dispositivo de una frecuencia a otra
	...
	//Empieza la recepción de datos pasando la función callback
	...
	//Si se desea se puede amplificar
	if (amp) {
	...
	//Una vez se ha inicializado parametros, se prepara el sweep y se inicia como receptor se habilita la antenna
	if (antenna) {
	...
	//En caso de que este recibiendo datos  se calcula la hora
	...
	//Una vez ha completado el sweep se cierra y se libera la correspondiente conexion
	if(device != NULL) {
